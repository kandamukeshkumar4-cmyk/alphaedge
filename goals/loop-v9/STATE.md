...
| 3 | 2026-07-10 | BE | N03 | edge history: `GET /api/v1/markets/{slug}/edge-history?window=` — bounded {t,model_p,market_p,edge} series from prediction+odds logs (at-or-before join), window-bounded + max_points cap, honest empty, desk-cache + M03 weak-ETag/304, auto-covered by 5xx guard | pytest 1286 passed / 28 skipped; ruff clean; 7 N03+guard tests green. AutoLab: not applicable (no iterative measure) |
| 4 | 2026-07-10 | FE | R01 | opportunities page: `/opportunities` route consuming N01 — ranked read-only "biggest edges right now" cards (edge chip ABSOLUTE via AnimatedNumber, model-vs-market, YES/NO lean, yes_price, liquidity, top_signal via SignalEvidenceBlock); direction (all/YES/NO) + min-liquidity SegTabs client-filter the one fetched set (no poll loop); honest empty ("no opportunities meet the filter right now"); added to header More nav + Discover Intelligence; new lib `opportunities-api.ts` (pure `buildOpportunitiesView`) + vitest. Analysis-only — no bet/execute CTA, only a paper-trade deep-link. | typecheck 0 err; lint 0 warn; vitest 44 files / 257 tests pass (12 new); next build OK (`/opportunities` 4.73 kB). AutoLab: not applicable (no iterative measure) |
| 5 | 2026-07-10 | FE | R02 | forecast drivers panel: new `ForecastDriversPanel` consuming N02, rendered inside the market-detail `DeskIntelligencePanel` as a "Why the model thinks this" section — signed model-vs-market gap chip + for/against driver rows (favors YES/NO/neutral badges, family labels, F04 citations via SignalEvidenceBlock); honest {found:false} / no-model / empty-drivers states, nothing fabricated; own fetch-on-load (no poll loop). New lib `drivers-api.ts` (pure `buildDriversView`) + vitest. | typecheck 0 err; lint 0 warn; vitest 45 files / 265 tests pass (8 new); next build OK. AutoLab: not applicable (no iterative measure) |
| 6 | 2026-07-10 | FE | R03 | edge history chart + a11y/polish closeout: new `EdgeHistoryChart` consuming N03 (SIGNED series) — inline-SVG two-line model_p vs market_p on a fixed 0..100% y-axis in the TrackRecordReliability style, `role="img"` + `<title>`/`<desc>`, latest signed edge via AnimatedNumber, null-market gaps break the line; rendered in `DeskIntelligencePanel`. New lib `edge-history-api.ts` (pure `buildEdgeHistoryView` + testable `buildLinePath`) + vitest. Closeout: skeletons on load across R01/R02/R03; AnimatedNumber on opportunity edge/stat tiles + chart edge; MotionReveal on opportunity cards; reduced-motion handled by AnimatedNumber/MotionReveal; aria-labelled sections; text-carried (not colour-only) direction/edge badges. Overflow @390px: chose cards (not a table) for /opportunities — single-column with truncated title/slug + flex-wrap chip rows = 0px horizontal overflow; SVG uses viewBox + w-full h-auto so it scales, never clips. | typecheck 0 err; lint 0 warn; vitest 46 files / 274 tests pass (9 new); next build OK (87/87 pages, exit 0). AutoLab: not applicable (no iterative measure) |
