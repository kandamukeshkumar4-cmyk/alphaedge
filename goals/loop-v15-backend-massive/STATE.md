...
| A6 | [LIVE] Order lifecycle soak | IN-PROGRESS | Started 2026-07-13T12:37:34-04:00.<br>Exists: A1 idempotent risk-gated submission, A3 atomic/idempotent cancellation, A5 authenticated history/fill breakdown, `LedgerService` balanced entries, and `Position` reconciliation. The committed tree is clean and integration `4e85533` is an ancestor.<br>Missing: one end-to-end running-server proof that rests a limit order, partially crosses it, cancels the remainder, and reconciles balances, ledger, positions, order/fill state, events, and history to the cent.<br>Plan: use isolated accounts/market/database; place both orders through real HTTP `POST /api/v1/markets/{slug}/orders`; verify partial history through real HTTP; cancel through real HTTP; reconcile database invariants independently. Docker client exists but Docker Desktop daemon is unavailable (`docker API` named pipe missing), so use real local Uvicorn with isolated SQLite; A6 has no worker-dependent behavior. Run the deterministic gate and require a fresh verifier before DONE. AutoLab: not applicable (no iterative measure). |
