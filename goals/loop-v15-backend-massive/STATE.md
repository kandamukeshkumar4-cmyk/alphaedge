...
| 039 | Workstream A | A4 | CLAIMED 2026-07-13T11:45:58-04:00 |
...
| `backend/app/db/models.py` | Workstream A | A4 | CLAIMED 2026-07-13T11:45:58-04:00 |
| `backend/app/schemas/market.py` | Workstream A | A4 | CLAIMED 2026-07-13T11:45:58-04:00 |
| `backend/app/api/v1/routes.py` | Workstream A | A4 | CLAIMED 2026-07-13T11:45:58-04:00 |
| `backend/app/workers/tasks.py` | Workstream A | A4 | CLAIMED 2026-07-13T11:45:58-04:00 |
...
| A4 | Order expiration (GTD) sweep | IN-PROGRESS | Started 2026-07-13T11:45:58-04:00.<br>Exists: CLOB `OrderIntent`, `OrderCreate`, `Order`, and `OrderBookService.cancel_order`; ARQ `WorkerSettings.functions/cron_jobs`; `JobRun` heartbeat rows; A3's atomic/idempotent cancellation path. No other Loop V15 worktree holds a live claim.<br>Missing: no optional `expires_at` contract or persisted column; no bounded expiry sweep; no JobRun-backed task/cron registration; no expired/live/idempotent tests.<br>Plan: claim migration 039 and the four shared order/task regions; add `expires_at` through intent/schema/route/model/service; implement bounded `workers/order_expiry.py` that cancels through A3 and records JobRun; minimally append its function+cron registration to `tasks.py`; test expired swept, future untouched, rerun idempotent, task heartbeat, and registration. Baseline: 36 focused tests passed. AutoLab: not applicable (no iterative measure). |
