...
| `backend/app/api/v1/routes.py` | Workstream A | A5 | CLAIMED 2026-07-13T12:05:54-04:00 |
| `backend/app/schemas/market.py` | Workstream A | A5 | CLAIMED 2026-07-13T12:05:54-04:00 |
...
| A5 | Order history/status API | IN-PROGRESS | Started 2026-07-13T12:05:54-04:00.<br>Exists: JWT human `PaperOrder` history at `GET /api/v1/orders/history`; CLOB account summary exposes only 25 terminal orders plus all open orders; `Fill` rows identify buy/sell order IDs, price, quantity, and timestamp. The two order ledgers are intentionally separate. No other Loop V15 worktree holds a live claim.<br>Missing: no authenticated, filterable, cursor-paginated CLOB `GET /api/v1/orders`; no per-order fill breakdown; no stable next-cursor contract.<br>Plan: claim only the GET-order regions of `routes.py` and `schemas/market.py`; add a new owned `services/order_history_service.py`; authenticate with the existing paper-account token rules; filter by status and market slug; use stable `(created_at, id)` keyset pagination; return fill rows for both maker/taker participation without changing existing endpoint shapes. Baseline: 24 focused tests passed. AutoLab: not applicable (no iterative measure). |
