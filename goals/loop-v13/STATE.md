...
| 5 | 2026-07-11 | FE | U02 degradation consistency + no-console-error audit | done | MEASURED: audited 7 V6-V10 data surfaces — ALL ALREADY CLEAN (0 gaps found, 0 fixes needed, a valid outcome). (a) /opportunities, /resolved, /compare, /categories/[category], /smart-money, /home, /s/[slug]: each degrades to an honest unreachable/not-found/empty state; every `fetch*` helper (opportunities/resolved/compare/category-summary/smart-money/share-snapshot/home-api) wraps the request in try/catch → returns `null` on error/!ok, and every `build*View` transform null-guards (`raw?.`, `!raw`, `reachable` flag) so no `.map` ever runs on undefined. (b) HealthBanner honest: polls /api/v1/health/detailed @30s, needs 2 consecutive bad probes before "unavailable" (no cold-start 429 flap), distinguishes degraded vs down, auto-clears on recovery. (c) Uncaught-error sweep: all 7 non-test `JSON.parse` sites — SSE/WS frame handlers (feed-api, useActivityFeed, useLiveMarket, useMarketPrice, NotificationBell) + localStorage reads (gamification, portfolio-store) — are try/catch-wrapped; no unguarded parse or `.map`-on-undefined found. No code change (audit-only); optional status affordance deliberately SKIPPED (new scope vs no-new-feature guardrail). Gate: typecheck clean; eslint clean (0 warnings); vitest 55 files / 341 tests passed; next build passed. AutoLab: not applicable (audit, no iterative measure). |
