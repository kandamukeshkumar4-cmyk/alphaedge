...
|------|------|-------|--------|--------|-------|
| 2 | 2026-07-10 | FE | W02 Alerts page + header bell | done | New `lib/alerts-api.ts` (J02 client: pure `buildAlertGroups` group-by-market + family filter, `alertFamilies`, `unreadAlertCount`, `newestAlertTs`, `relativeTime`; reuses `categorizeSignal` + `extractSignalEvidence`), `components/AlertsBell.tsx` (header bell, fetch-once no-poll, unread vs localStorage `alertsLastSeenTs`, links `/alerts`) added to SiteHeader. Rebuilt `/alerts` around J02: groups by market, `SignalEvidenceBlock` evidence, family filter pills, watchlist-scoped when authed / public when anon, honest "No alerts yet" empty; marks-seen on mount to clear the bell. Gate: typecheck ✓, lint ✓ (0 warn), vitest 36 files/185 tests ✓ (alerts-api.test.ts 8 cases: grouping/family/unread/relativeTime), build ✓ (`/alerts` 7.33 kB). 390px: flex-wrap pills + truncate slug + shrink-0 badges, no overflow. Degrade note: J02 items assumed `{id,signal_type,platform,market_id,payload,created_at}` with H03 citation fields inside `payload`; evidence renders null when absent. Old dispatched-alerts/engine-room page replaced (no test pinned it; activity-api untouched, still used by /signals). AutoLab: baseline=green FE gate | benchmark=vitest+build green | iterations=1 | budget=1/3 | outcome=improved (alerts feed + bell shipped) |
| 1 | 2026-07-10 | FE | W01 Watchlist UI | done | New `lib/watchlist-api.ts` (pure view-model + `toggleWatchlistSlug` + JWT GET/POST/DELETE), `hooks/useWatchlist.ts` (shared optimistic state, reverts on failed mutation, `"anon"`→sign-in), `components/WatchlistStar.tsx` (aria-pressed ☆ toggle) wired into QuestMarketCard + market detail header, new `/watchlist` page (edge + last-move rows; honest "sign in to track markets" anon / "no markets yet" empty). Gate: typecheck ✓, lint ✓ (0 warn), vitest 35 files/177 tests ✓ (incl. watchlist-api.test.ts 8 cases: view-model + optimistic toggle), build ✓ (`/watchlist` static, 5.37 kB). 390px: truncate title + `hidden sm:block` edge col, no overflow. Degrade note: J01 GET shape assumed `{items:[{slug,title,snapshot{yes_price,last_move_pts},edge{edge_vs_book}}]}`; all fields optional and rendered "—" when absent. AutoLab: baseline=green FE gate | benchmark=vitest+build green | iterations=1 (first pass green) | budget=1/3 | outcome=improved (watchlist shipped) |
| 3 | 2026-07-10 | FE | W03 | DONE (orchestrator-completed after builder hit session limit) — Model A/B card on /eval: new lib/model-ab-api.ts (pure buildModelAbView tolerant of absent fields + fetchModelAb/fetchResolvedCount) + ModelAbCard.tsx: progress bar toward the 100-resolve threshold, XGB-vs-LGBM Brier comparison when ready, explicit "default unchanged — analysis only" note, honest not-ready + lgbm-unavailable states; fetch-once on mount (no poll). Mounted on /eval. | typecheck ✓ lint ✓ vitest 191 passed (incl. model-ab view-model tests) ✓ build ✓. AutoLab: not applicable (additive readout card). |
