...
| `backend/app/workers/tasks.py` | V4 | RELEASED 2026-07-14T11:17:10-04:00 — minimal additive import/function/cron only; parallel Workstream D D2 claim acknowledged |
...
Diagnosis: the production read-only endpoint returned `resolved_count=1`,
`ab_threshold=100`, and `ab_ready=false`. The repository already had a
default-on auto-lock setting, real venue snapshots, immutable LIVE forecast
locking, resolver grading, and the leakage gate, but no scheduled worker ever
selected eligible pre-close external markets and created those lock records.
The grading pipeline therefore had almost nothing to resolve.

```json
{"resolved_count":1,"ab_threshold":100,"ab_ready":false,"model_default":"xgboost","paper_trading_only":true}
```

Fix: added a bounded worker that selects only OPEN external markets closing in
the configured window and lacking a LIVE forecast, fetches a real venue
snapshot, runs the existing model prediction and immutable lock path, and uses
a dedicated non-authenticatable system forecaster. It processes each market in
a savepoint, caps every batch, and records success/degraded/failed JobRun
heartbeats. The shared `workers/tasks.py` change is limited to the claimed
additive import, task/cron registration, and settings exposure.

The first fresh-context review found a real time-of-check/time-of-use defect:
the venue could become terminal on the second snapshot fetched by
`lock_forecast`, or the market close time could move ahead of the actual lock
timestamp. The final iteration re-checks the persisted snapshot metadata and
`locked_at < close_at` inside the savepoint, rolling back the snapshot,
forecast, and event when eligibility is lost. Regression tests cover both
races. No scoring threshold, order path, or leakage constraint changed.

Task gate: focused tests `33 passed in 9.87s`; full backend `1426 passed, 28
skipped in 230.49s`; Ruff `All checks passed!`.

```text
=== GATE: backend pytest ===
1426 passed, 28 skipped in 252.66s (0:04:12)
PASS backend pytest (exit 0)
...
Windows pytest temp-directory cleanup warnings occurred after exit 0 and are
non-blocking.

Fresh-context verifier: PASS after the review-driven race fix — 33 focused
tests passed in 23.45s, Ruff and `git diff --check` passed, and every scoped
line was inspected. The verifier confirmed bounded and locked selection,
pre-close/terminal re-checks, rollback isolation, task/cron/settings
registration, heartbeat outcomes, non-authenticatable forecaster credentials,
and preservation of the leakage and paper-only boundaries. No blocking
findings remain.

Manual code-review fallback used because the Superpowers
`requesting-code-review` skill is unavailable; the complete diff was reviewed
with the initial race finding fixed. `gh-address-comments`: not applicable (no
PR/merge). Bumblebee: not applicable (no manifest, lockfile, dependency loader,
or deployment-image change; no merge requested).
