...
|------|------|-------|--------|--------|-------|
| 6 | 2026-07-10 | FE | T03 | Full a11y + 390px sweep of all 39 routes: added a global keyboard focus ring + 3 chart/toggle aria gaps; 390px overflow already clean | **Audited every `src/app/**` route** (39 pages) across the 3 axes. FINDINGS + FIXES: (1) **Keyboard focus visibility — the one systemic gap**: `globals.css` had **no `:focus-visible` rule at all**, so keyboard focus on native buttons/links/icon-buttons/tabs across all routes was invisible or UA-inconsistent. Fixed with a single global `:where(a,button,input,select,textarea,summary,[role=button|tab|combobox|switch],[tabindex]):focus-visible { outline: 2px solid #00C9A0 }` — `:where()` = specificity 0 so every existing component focus style (the 9 inputs already using `focus:border-accent`, plus `focus-within` containers) still wins; `:focus-visible` = keyboard-only so pointer/visual appearance is byte-identical. Plus 1 genuinely-bare interactive opt-out: `FeedFilterBar` platform `<select>` had `focus:outline-none` with no affordance → added `focus-visible:border-accent` + `aria-label="Filter feed by platform"`. (`HeaderSearch` + `AtlasPanel` inputs verified clean — their containers already use `focus-within:border-*`.) (2) **Chart aria/role**: `EquityCurveChart` + `OrderbookDepthChart` already had `role="img"`+label; added `role="img"`+`aria-label` (live price / probability) to the 2 that lacked it — `PriceChart` and `ProbabilityHistoryChart` container divs. (3) **390px page-level horizontal overflow: already clean** — all 10 `<table>` (+wide `min-w-[420–760px]` blocks) are inside `overflow-x-auto` containers (prior D04 pass); no other page-level fixed width ≥400px found; the T01 lazy wrappers reserve `min-height` only (no width). No behaviour/UX change; no test weakened. Gate: typecheck ✓, lint ✓ (0 warnings), test 330 passed / 53 files, build ✓ 100 pages. AutoLab: baseline=330 tests + build green \| benchmark=a11y issues fixed (focus ring routes + aria gaps) + 390px overflow count \| iterations=1 (1 global focus fix covering 39 routes, 1 select, 2 chart aria; 0 overflow issues) \| budget=1/1 \| outcome=improved |
| 5 | 2026-07-10 | FE | T02 | Coverage broadening: +28 vitest cases across 4 previously-untested pure/trade-flow lib modules | Added unit tests for modules with **zero prior coverage**: `portfolio-store.test.ts` (paper trade-flow helper — installs a fake window+localStorage+event bus to exercise the real persistence path: `placeOrder` share/balance validation branches, balance deduction + position/history prepend, subscribe/unsubscribe notify, `resetPortfolio`, corrupt-JSON fallback), `market-href.test.ts` (static-seed vs live-slug routing for `marketHref`/`marketIntelHref`/`briefHref` incl. query-param encoding), `market-sources.test.ts` (`isKalshi/PolymarketSource` source+slug-prefix logic and the two filters), `team-flags.test.ts` (`flagForTeam` exact / substring / tie-draw / fallback). No existing test weakened or deleted. Test-count delta: **302 → 330 (+28)**; files **49 → 53 (+4)**. Gate: typecheck ✓, lint ✓ (0 warnings), test 330 passed / 53 files, build ✓ 100 pages. AutoLab: baseline=302 tests (green) \| benchmark=vitest case count on critical lib/trade-flow logic \| iterations=1 (+28) \| budget=1/1 \| outcome=improved |
