...
| E09 | **C1/C2 schema consolidation** (PaperOrder/Order, PaperSignal fold). | DEFERRED (again, deliberately) 2026-07-03 | Deferred with reasons, not silently: (1) it was explicitly signed off as too-risky in the original loop; (2) zero user-facing value — it can't make anything more visible or more live; (3) the branch is shared with another active loop (loop-c) committing to the same schema area — a cross-table consolidation now maximizes collision risk; (4) suite is green as-is. Revisit when the branch is single-writer and quiet. |
