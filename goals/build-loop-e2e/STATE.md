...
| AUD-06b | **Session model phase 2** (H-SEC-02 full / M-SEC-03): drop localStorage token, cookie-only auth on all fetches, CORS allow_credentials exact-origin, CSRF on mutations | TODO | needs live cross-origin login/admin verification; latent-not-exploitable today (no HTML-injection surface, I-SEC-01) |
