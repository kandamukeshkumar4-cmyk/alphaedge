...
| E09 | **C1/C2 schema consolidation** (PaperOrder/Order, PaperSignal fold). | RESOLVED — no dedup warranted 2026-07-03 | Investigated the actual models (not just re-deferred). **They are NOT duplicates**: `PaperOrder` = JWT-user paper-trade ledger (user_id, slug, cost, realized_pnl, settled); `Order` = CLOB matching-engine order (account_id, order_type, price/quantity, filled_quantity, fills). `PaperSignal` = per-account watchlist marker; `signal_events` = pipeline DeltaEvents (signal_type/platform/payload). Different producers, schemas, lifecycles, consumers. Enums (OrderSide/OrderOutcome) are ALREADY shared — no duplication there either. "Consolidating" them would merge distinct domain concepts = an architectural error, not a cleanup (matches the prior principal-eng sign-off). **Fix applied**: added clarifying docstrings to `PaperOrder` and `PaperSignal` documenting why they're intentionally separate, so this stops being flagged as debt. Ticket closed, not hanging. |
