...
| REL-FE-CUTOVER | **Frontend prod cutover to Railway**: Vercel must serve `next.config.ts` PROD_API → Railway (or set `NEXT_PUBLIC_API_URL`) | DONE 2026-07-13 (via Vercel CLI, user account authed) | Root cause the code default wasn't enough: Vercel had **`NEXT_PUBLIC_API_URL` + `NEXT_PUBLIC_WS_URL` pinned to the HF Space** (set 6d ago), which override the next.config default AND drive the client WS URL. Fix: `vercel env rm`+`add` both → `https://alphaedge-api-production-b9db.up.railway.app` / `wss://…railway.app`, then `vercel --prod --yes` (fresh build inlines NEXT_PUBLIC_*). **Live-verified**: `alphaedge-frontend-three.vercel.app/health` returns Railway's `paper_trading_only:true` (was HF 503); homepage renders **132 market cards, 0 console errors**; `verify_prod` check 5 PASS (98/98 pm slugs in HTML). **verify_prod now 5/6** — only check 2 (markets ≥200) unmet: fresh Railway DB has 126 HIGH-VOLUME real markets (WC/elections); not padding with low-vol junk to hit a count calibrated for the aged HF DB. |
