...
| O06 | **Deep daily digest on reasoning model** (was B05): route the daily digest to `LLM_MODEL_DEEP` (nemotron-49b class, ~19s fine for cron); per-feature model routing in config; digest cites news/whale/macro evidence. | DONE (plumbing) 2026-07-06 · live run deferred to O01 | `llm_model_deep` setting (empty→falls back to LLM_MODEL, opt-in); `resolve_llm_model(settings, deep=)` in provider.py; `deep` flag threaded AnalystState→run_analyst→run_analyst_for_trigger→write_brief (uses resolved model id, warning log now names it); ResearchDigestService.run_daily calls the analyst with deep=True (digest is cron/latency-tolerant, interactive stays fast). `LLM_MODEL_DEEP` added to deploy-hf-space secret sync. 6 tests (resolver default/deep-fallback/deep-set/whitespace + write_brief sends deep model / stays fast when not deep). ruff clean; analyst+daily_brief suites green (22 passed). **Live NIM run still needs O01's key** — set LLM_MODEL_DEEP as a GH secret + a working HF deploy to verify the digest runs on the reasoning model. |
