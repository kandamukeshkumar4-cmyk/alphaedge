...
| 6 | 2026-07-10 | FE | Y03 | green | typecheck+lint clean, vitest 235 passed (41 files; +12 new notify-prefs-api.test.ts), build OK. New lib notify-prefs-api.ts (buildNotifyPrefsView + optimistic toggleFamily + isCategoryEnabled/filterItemsByPrefs + authed GET/PUT). New NotifyPrefs panel on /alerts: per-family aria-labelled checkboxes, optimistic PUT with rollback-on-failure (the ONLY mutation — a stored pref, JWT Bearer, no external delivery), honest anon sign-in prompt + unreachable state. Stored prefs constrain the /alerts feed families AND the header bell unread count (both pass-through when null/anon). 390px: toggle rows min-w-0 truncate label + shrink-0 checkbox/status → no overflow. AutoLab: baseline=Y02 green | benchmark=optimistic toggleFamily + anon/unreachable view + prefs-filter cases | iterations=1 | budget=1/3 | outcome=improved |
| 7 | 2026-07-10 | FE | Y04 | green | typecheck+lint clean, vitest 235 passed (41 files), build OK. a11y/consistency closeout on the 3 new surfaces. Findings + fixes: (1) backtest metric tiles rendered counts as plain strings while n_bets/n now respond to params → brought onto the AnimatedNumber idiom (Resolved n + bets count, Tile widened to ReactNode). (2) verified all three have skeletons on load (SelfServeBacktest, DigestSkeleton, PrefsSkeleton), MotionReveal reveals (digest+prefs cards), AnimatedNumber on counts (digest total/family/mover counts + backtest counts). (3) aria-labels present: backtest chart SVG role=img+aria-label+title, number inputs aria-label, digest window toggle role=group aria-label + per-button aria-pressed+aria-label, prefs checkboxes per-family aria-label, saving status role=status + error role=alert. (4) reduced-motion: global globals.css @media(prefers-reduced-motion:reduce) neutralizes ALL animations incl. skeleton shimmer; MotionReveal + AnimatedNumber additionally gate on useReducedMotion — respected. (5) 390px overflow: reasoned per surface — backtest params row flex-wrap basis-40 min-w-0 stacks; digest family pills flex-wrap, mover rows min-w-0 truncate slug + shrink-0 count, family/time badges sm:inline-only (hidden <640px); prefs toggle rows min-w-0 truncate label + shrink-0 checkbox/status; all inputs w-full → 0px horizontal overflow. NOTE: browser 390px check n/a — the running preview server cwd is the main repo (E:\polymarket clone), not this worktree; verified statically + via passing next build. AutoLab: not applicable (no iterative measure — a11y/consistency closeout). |
