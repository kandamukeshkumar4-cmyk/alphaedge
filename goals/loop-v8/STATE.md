...
| 6 | 2026-07-10 | FE | Z03 | done | Wire-in + a11y + perf closeout. Added a "Share ↗" affordance to the market-detail `DeskIntelligencePanel` header linking to `/s/[slug]` (aria-label "Open shareable snapshot for this market"); Home reachable from header nav (Z01). Findings/fixes: (a) share card raw slug had no wrap → long live `pm-*` slugs could overflow at 390px; hardened title with `break-words` and slug with `break-all`. (b) 390px review of both surfaces — /home grids are single-column below lg, all list rows use `min-w-0`+`truncate` on the flexible slug/title with `shrink-0` chips, digest/edge chip rows `flex-wrap`, model progress bar `w-full`; /s uses `max-w-2xl` in the narrow shell with `flex-wrap` on the edge line and action row → 0px horizontal overflow. (c) a11y — new sections carry `aria-label` (Top signals) or heading (Panel h3); copy button + share link labelled; progress bar `aria-hidden` with the resolved count exposed as text. (d) motion — MotionReveal + AnimatedNumber (counts) both already honor prefers-reduced-motion; share price intentionally static (single-shot snapshot, no live tween). Skeletons on load for both routes. Note: preview server runs against the main repo, not this worktree, so verification was static reasoning + the build gate. Gate: typecheck ✓, lint ✓ (0 warnings), vitest 247 passed (43 files), build ✓ (/home 7.26 kB, /s/[slug] 4.22 kB). AutoLab: not applicable (one-shot wire-in/a11y, no iterative measure). |
