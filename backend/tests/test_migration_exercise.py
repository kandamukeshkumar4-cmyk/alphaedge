...
BASELINE_README = (
    "Known, pre-existing model/DDL drift. Generated by "
    "tests/test_migration_exercise.py with UPDATE_MIGRATION_PARITY_BASELINE=1. "
    "Entries are 'DRIFT | OBJECT | DETAIL'. Baselined at 125 in loop111; "
    "revision 067_notnull_parity closed 64 of them in loop112 (63 columns that "
    "carried a model default could be backfilled and SET NOT NULL, plus the "
    "markets(slug) unique constraint, which was adopted into the model). "
    "What is left is (a) NULLABILITY MISMATCH on columns with no safe default "
    "- FKs, measurements and identity/free-text fields where a legacy NULL "
    "cannot be invented, so they need a data audit before SET NOT NULL - and "
    "(b) TYPE MISMATCH 'db=JSONB models=JSON': the DATABASE is already the "
    "better type; the models declare plain JSON. Closing those means changing "
    "app/db/models.py to JSON().with_variant(JSONB(), 'postgresql'), not a "
    "migration. Shrink this list when drift is fixed - never grow it to "
    "silence a new mismatch."
)
