...
failure, printing PASS/FAIL per check. Run:  py -3.13 scripts/verify_prod.py
  ...
  5. GET frontend homepage (+JS chunks) -> >=3 catalog "pm-" slugs AND no "alpha_quant"
...
def main() -> int:
    ...
    # ---- Check 5: frontend homepage live-data-first (anti-mock) ----
    try:
        catalog_pm = sorted(
            {m["slug"] for m in (markets or []) if str(m.get("slug", "")).startswith("pm-")}
        )
        status, body = http_get(f"{fe}/")
        html = _decode(body)
        combined = html

        # collect script srcs and fetch JS chunks (SPA: data may be baked into a chunk)
        ...
        if catalog_pm:
            found = {s for s in catalog_pm if s in combined}
            n_found = len(found)
            where = f"of {len(catalog_pm)} catalog pm- slugs"
        else:
            found = set(re.findall(r"pm-[a-z0-9][a-z0-9-]+", combined))
            n_found = len(found)
            where = "pm- slugs via regex (no catalog)"

        alpha_present = "alpha_quant" in combined
        ok = n_found >= 3 and not alpha_present
        if not ok:
            if alpha_present:
                msg = f"fake trades marker 'alpha_quant' present; slugs {n_found} {where}"
            else:
                msg = f"homepage not live-data-first ({n_found} {where})"
        else:
            msg = f"{n_found} {where} in HTML+{fetched} chunks; no alpha_quant"
